...
const NetworkingModuleRenderer: React.FC<Props> = ({ moduleId, page }) => {
  const [courseData, setCourseData] = useState<any>(null);

  // Loaded on demand so the course content stays out of the main bundle.
  useEffect(() => {
    import('./NetworkingCourseData').then((module) => {
      setCourseData(module.NETWORKING_COURSE_DATA);
    });
  }, []);
  ...
  if (page === totalLessons + 1) {
    return <ModuleQuiz key={moduleId} moduleId={`networking-${moduleId}`} questions={moduleData.quiz || []} />;
  }
  ...
  // Most examples are runnable Python that computes something by hand —
  // subnet maths, checksums, window arithmetic. Lessons whose example is a
  // capture or a config override this with their own `language`.
  return (
    <StandardLessonView
      lesson={lessonData}
      language={lessonData.language || 'python'}
      snippetTitle={lessonData.snippetTitle || 'example.py'}
    />
  );
};

export default NetworkingModuleRenderer;
