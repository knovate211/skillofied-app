...
const OsModuleRenderer: React.FC<Props> = ({ moduleId, page }) => {
  const [courseData, setCourseData] = useState<any>(null);

  // Loaded on demand so the course content stays out of the main bundle.
  useEffect(() => {
    import('./OsCourseData').then((module) => {
      setCourseData(module.OS_COURSE_DATA);
    });
  }, []);
  ...
  if (page === totalLessons + 1) {
    return <ModuleQuiz key={moduleId} moduleId={`os-${moduleId}`} questions={moduleData.quiz || []} />;
  }
  ...
  // Systems C++ is the default: the sandbox compiles with g++ and genuinely
  // runs fork, exec, pthreads and signals, so kernel-interface examples are
  // real rather than illustrative. Lessons whose example is a shell session or
  // a capture override this with their own `language`.
  return (
    <StandardLessonView
      lesson={lessonData}
      language={lessonData.language || 'cpp'}
      snippetTitle={lessonData.snippetTitle || 'example.cpp'}
    />
  );
};

export default OsModuleRenderer;
